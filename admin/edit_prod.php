...
if(isset($_POST['edit_prod'])){
    ...
    if (empty($prod_title) || empty($prod_desc) || empty($prod_feat1) || empty($prod_feat2) || empty($prod_feat3) || 
    empty($prod_key) || empty($subcategory) || empty($prod_price) || empty($prod_img1) || empty($prod_img2) || empty($prod_dis)) {
        ...
        $update_query = "UPDATE `product` SET prod_title=?, prod_desc=?, prod_feat1=?, prod_feat2=?, prod_feat3=?, prod_key=?, 
        subcat_id=?, prod_img1=?, prod_img2=?, prod_price=?, prod_discount=?, prod_date=NOW() WHERE prod_id=?";

$stmt = mysqli_prepare($con, $update_query);
mysqli_stmt_bind_param($stmt, "ssssssisssii", $prod_title, $prod_desc, $prod_feat1, $prod_feat2, $prod_feat3, $prod_key,
 $subcategory, $prod_img1, $prod_img2, $prod_price, $prod_dis, $edit_id);
